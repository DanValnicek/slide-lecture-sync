...
class SlideMatcher:
    ...
    def matched_slide(self, frame, debug_info: list = None) -> (int, int):
        ...
        for slide_idx, slide_matches in sorted(instance_cnt.items(), key=lambda x: len(x[1]), reverse=True):
            ...
            matches = bfm.knnMatch(desc2, self.descriptors[slice(*self.slideIdxToDescRange(slide_idx))], k=1)
            if len(matches) < 5:
                del instance_cnt[slide_idx]
                continue
            range_s, range_e = self.slideIdxToDescRange(slide_idx)
            dataset_keypoints = self.keypoints[range_s:range_e]
            # db_tf_idf = self.dataset_tf_idf[range_s:range_e]
            # db_idf = self.dataset_idf[range_s:range_e]
            # db_tf_idf = np.float32([db_tf_idf[m[0].trainIdx] for m in matches])
            # db_idf = np.float32([db_idf[m[0].trainIdx] for m in matches])
            ...
            patch_cnt = self.patch_coverage2(src_kps2, s_w, s_h)
            patch_results[slide_idx] = patch_cnt
